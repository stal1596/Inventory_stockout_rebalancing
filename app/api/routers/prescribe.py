...
def build_recommendations(dataset, network, scored: pd.DataFrame,
                          horizon: int = DEFAULT_HORIZON) -> pd.DataFrame:
    """Run the engine over every open position. Called once, at startup.

    Per-request runs cost 3.4 s even for a single SKU, because the engine rescans
    the 525k-row panel to find today's stock and the donor pool. Doing it once
    also means the recommendation list and the per-SKU detail are literally the
    same rows, so they cannot drift apart.
    """
    return engine.recommend(
        attach_context(dataset, scored), dataset, network,
        horizon=horizon, n_paths=250,
    )
...
def recommendations(
    ...
    for _, row in result.head(limit).iterrows():
        story = _story(row, horizon)
        rows.append(
            {
                "store_id": row["store_id"],
                "sku_uid": row["sku_uid"],
                "action": row["recommended_action"],
                "action_label": LEVER_LABELS.get(
                    row["recommended_action"], row["recommended_action"]
                ),
                "speed": LEVER_SPEED.get(row["recommended_action"], "-"),
                ...
                "baseline_lost_units": round(
                    float(row["baseline_expected_lost_units"]), 1
                ),
                **story,
            }
        )
    ...
    return {
        ...
        "rows": rows,
        "mix": records(summary),
        "no_action_share": round(float((~acted).mean()), 4),
        "total_net_value": round(float(result.loc[acted, "expected_net_value"].sum()), 0),
        "caveat": (
            "A transfer is invisible in this extract: nothing records inter-store "
            "movement. Executing one without capturing it degrades the data, and "
            "model metrics will improve while real stockouts are hidden."
        ),
    }
